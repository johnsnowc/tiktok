...
var (
	...
	videoRowsExpectAutoSet   = strings.Join(stringx.Remove(videoFieldNames, "`id`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`"), ",")
	videoRowsWithPlaceHolder = strings.Join(stringx.Remove(videoFieldNames, "`id`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`"), "=?,") + "=?"
...
type (
	...
	Video struct {
		...
		DeleteTime    sql.NullInt64 `db:"delete_time"`
	}
)
...
func (m *defaultVideoModel) Insert(ctx context.Context, data *Video) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Uid, data.PlayUrl, data.CoverUrl, data.FavoriteCount, data.CommentCount, data.Title, data.DeleteTime)
	}, videoIdKey)
	return ret, err
}
...
func (m *defaultVideoModel) Update(ctx context.Context, data *Video) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Uid, data.PlayUrl, data.CoverUrl, data.FavoriteCount, data.CommentCount, data.Title, data.DeleteTime, data.Id)
	}, videoIdKey)
	return err
}
